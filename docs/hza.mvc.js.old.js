gin.ns('hza.router', {
  ...
  route: function (path) {
    ...
    this._view = segments[1];
    this._resourceId = segments[2] ? segments[2] : null;
    this._getResource(this._controller, this._view);
    this.loadCurrent();
  },

  _getResource: function (controller, view) {
    if (this.controllers[controller] && this.controllers[controller][view]) {
      this.unloadCurrent();
      this.current = this.controllers[controller][view];
    } else {
  ...
  loadCurrent: function () {
    this.current(this._resourceId);
  },

  unloadCurrent: function () {
    if (!this.current) { return; }
    this.controllers[this._controller].getView(this._view).unload();
  },
